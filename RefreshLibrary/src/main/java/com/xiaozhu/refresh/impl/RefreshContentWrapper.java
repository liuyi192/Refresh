...
import com.xiaozhu.refresh.util.ScrollBoundaryUtil;
...
public class RefreshContentWrapper implements RefreshContent {
    ...
    private void findScrollableView(View content, RefreshKernel kernel) {
        ...
        if (mScrollableView instanceof NestedScrollingParent && !(mScrollableView instanceof NestedScrollingChild)) {
    ...
    private boolean isNestedScrollingChild(View targetView, MotionEvent event) {
        if ((targetView instanceof NestedScrollingChild || Build.VERSION.SDK_INT >= 21) && (Build.VERSION.SDK_INT >= 21 && targetView.isNestedScrollingEnabled())) {
            return true;
        }
    ...
    private class Api23ViewScrollComponent implements View.OnScrollChangeListener {
        ...
        public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
            ...
            if (scrollY <= 0 && oldScrollY > 0 && mMotionEvent == null && lastTime - lastTimeOld > 1000 && overScroll && layout.isEnableRefresh()) {
                ...
                if (!layout.isLoadmoreFinished() && layout.isEnableAutoLoadmore() && layout.getState() == RefreshState.None && !ScrollBoundaryUtil.canScrollDown(v)) {
    ...
    private class NestedScrollViewScrollComponent implements NestedScrollView.OnScrollChangeListener {
        ...
        public void onScrollChange(NestedScrollView scrollView, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
            ...
            if (scrollY <= 0 && oldScrollY > 0 && mMotionEvent == null && lastTime - lastTimeOld > 1000 && overScroll && layout.isEnableRefresh()) {
                ...
                if (!layout.isLoadmoreFinished() && layout.isEnableAutoLoadmore() && layout.getState() == RefreshState.None && !ScrollBoundaryUtil.canScrollDown(scrollView)) {
                    kernel.getRefreshLayout().autoLoadmore(0, 1);
        ...
        public void onScroll(AbsListView absListView, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (totalItemCount > 0) {
                ...
                if (mMotionEvent == null && dy > 0 && firstVisibleItem == 0) {
                    if (overScroll && layout.isEnableRefresh() && !ScrollBoundaryUtil.canScrollUp(absListView)) {
                        kernel.animSpinnerBounce(Math.min(dy, mHeaderHeight));
                    }
                } else if (dy < 0) {
                    int lastVisiblePosition = absListView.getLastVisiblePosition();
                    if (lastVisiblePosition == totalItemCount - 1 && lastVisiblePosition > 0 && layout.isEnableLoadmore()) {
                        if (!layout.isLoadmoreFinished() && layout.isEnableAutoLoadmore() && layout.getState() == RefreshState.None && !ScrollBoundaryUtil.canScrollDown(absListView)) {
                            kernel.getRefreshLayout().autoLoadmore(0, 1);
    ...
    private class RecyclerViewScrollComponent extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (newState == RecyclerView.SCROLL_STATE_IDLE && mMotionEvent == null) {
                ...
                } else if (layout.isEnableLoadmore() && !layout.isLoadmoreFinished() && layout.isEnableAutoLoadmore() && layout.getState() == RefreshState.None) {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            lastDy = dy;
            RefreshLayout layout = kernel.getRefreshLayout();
            if (dy > 0 && layout.isEnableLoadmore() && !layout.isLoadmoreFinished() && layout.isEnableAutoLoadmore() && layout.getState() == RefreshState.None) {
                ...
                if (lastVisiblePosition >= manager.getItemCount() - 1 && lastVisiblePosition > 0 && !ScrollBoundaryUtil.canScrollDown(recyclerView)) {
                    kernel.getRefreshLayout().autoLoadmore(0, 1);
                }
            }
        }
