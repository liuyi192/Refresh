...
import com.xiaozhu.refresh.api.DefaultRefreshFooterCreater;
import com.xiaozhu.refresh.api.DefaultRefreshHeaderCreater;
import com.xiaozhu.refresh.api.RefreshContent;
...
import com.xiaozhu.refresh.constant.DimensionStatus;
...
import com.xiaozhu.refresh.header.BezierRadarHeader;
import com.xiaozhu.refresh.header.FalsifyHeader;
import com.xiaozhu.refresh.impl.RefreshContentWrapper;
import com.xiaozhu.refresh.impl.RefreshFooterWrapper;
import com.xiaozhu.refresh.impl.RefreshHeaderWrapper;
import com.xiaozhu.refresh.listener.OnLoadmoreListener;
import com.xiaozhu.refresh.listener.OnMultiPurposeListener;
import com.xiaozhu.refresh.listener.OnRefreshListener;
import com.xiaozhu.refresh.listener.OnRefreshLoadmoreListener;
import com.xiaozhu.refresh.util.DensityUtil;
import com.xiaozhu.refresh.util.ViscousFluidInterpolator;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mRefreshContent != null) {
            ...
            final int widthSpec = getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight() + lp.leftMargin + lp.rightMargin, lp.width);
            final int heightSpec = getChildMeasureSpec(heightMeasureSpec, getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin + ((isInEditMode && mRefreshHeader != null && (mEnableHeaderTranslationContent || mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind)) ? mHeaderHeight : 0) + ((isInEditMode && mRefreshFooter != null && (mEnableFooterTranslationContent || mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind)) ? mFooterHeight : 0), lp.height);
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        if ((reboundAnimator != null && !interceptAnimator(action)) || (mState == RefreshState.Loading && mDisableContentWhenLoading) || (mState == RefreshState.Refreshing && mDisableContentWhenRefresh)) {
            return false;
        }
        if (mNestedScrollInProgress) {//嵌套滚动时，补充竖直方向不滚动，但是水平方向滚动，需要通知 onHorizontalDrag
            ...
            return ret;
        } else if (!isEnabled() || (!mEnableRefresh && !mEnableLoadmore) || (mHeaderNeedTouchEventWhenRefreshing && (mState == RefreshState.Refreshing || mState == RefreshState.RefreshFinish)) || (mFooterNeedTouchEventWhenRefreshing && (mState == RefreshState.Loading || mState == RefreshState.LoadFinish))) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if ((mRefreshContent != null) && (getViceState().isHeader() && (spinner < 0 || mLastSpinner < 0)) || (getViceState().isFooter() && (spinner > 0 || mLastSpinner > 0))) {
    ...
    protected boolean overSpinner() {
        ...
        } else if (mState == RefreshState.PullDownToRefresh || (mEnablePureScrollMode && mState == RefreshState.ReleaseToRefresh)) {
            setStatePullDownCanceled();
        } else if (mState == RefreshState.PullToUpLoad || (mEnablePureScrollMode && mState == RefreshState.ReleaseToLoad)) {
    ...
    protected void moveSpinner(int spinner, boolean isAnimator) {
        ...
        if ((spinner >= 0 || oldSpinner > 0) && mRefreshHeader != null) {
            ...
            if (mEnableRefresh || (mState == RefreshState.RefreshFinish && isAnimator)) {
                if (oldSpinner != mSpinner && (mRefreshHeader.getSpinnerStyle() == SpinnerStyle.Scale || mRefreshHeader.getSpinnerStyle() == SpinnerStyle.Translate)) {
        ...
        if ((spinner <= 0 || oldSpinner < 0) && mRefreshFooter != null) {
            ...
            if (mEnableLoadmore || (mState == RefreshState.LoadFinish && isAnimator)) {
                if (oldSpinner != mSpinner && (mRefreshFooter.getSpinnerStyle() == SpinnerStyle.Scale || mRefreshFooter.getSpinnerStyle() == SpinnerStyle.Translate)) {
            ...
            public void run() {
                if (mState == RefreshState.Loading) {
                    if (mRefreshFooter != null && mKernel != null) {
                        ...
                        notifyStateChanged(RefreshState.LoadFinish);
                        ValueAnimator.AnimatorUpdateListener updateListener = mRefreshContent.onLoadingFinish(mKernel, mFooterHeight, startDelay, mReboundInterpolator, mReboundDuration);
                        if (mOnMultiPurposeListener != null) {
